...
int32_t FICA_app_program_ext_init(int32_t newimgtype, uint8_t erase)
{
    status_t status;
...
//    }
    ...
    if (SLN_FLASH_NO_ERROR == status && erase)
    {
...
int32_t FICA_Erase_Bank(uint32_t startaddr, uint32_t banksize)
{
    ...
    uint32_t sizeIncrement = (startaddr & EXT_FLASH_ERASE_BLOCK -1)?EXT_FLASH_ERASE_PAGE:EXT_FLASH_ERASE_BLOCK;
    status_t (*erase_fun)(uint32_t) = (startaddr & EXT_FLASH_ERASE_BLOCK -1)?SLN_Erase_Sector:SLN_Erase_Block;
#else
    uint32_t sizeIncrement = EXT_FLASH_ERASE_PAGE;
    status_t (*erase_fun)(uint32_t) = SLN_Erase_Sector;
    ...
    for (uint32_t runaddr = startaddr; runaddr < (startaddr + banksize); runaddr += sizeIncrement)
    {
        ...
        if (erase_fun(runaddr) != kStatus_Success)
            return (SLN_FLASH_ERROR);
    }
