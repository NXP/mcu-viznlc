...
/**
 * @brief declare the lpm dev ##name
 */
#define HAL_LPM_DEV_DECLARE(name) int HAL_LpmDev_##name##_Register();

/**
 * @brief register the lpm dev ##name
 */
#define HAL_LPM_DEV_REGISTER(name, ret)                             \
    {                                                               \
        ret = HAL_LpmDev_##name##_Register();                       \
        if (ret != 0)                                               \
        {                                                           \
            LOGE("HAL_LpmDev_%s_Register error %d", "##name", ret); \
            return ret;                                             \
        }                                                           \
    }
