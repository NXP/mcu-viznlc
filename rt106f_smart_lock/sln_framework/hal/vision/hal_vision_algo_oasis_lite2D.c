...
AT_NONCACHEABLE_SECTION_ALIGN_DTC(static uint8_t s_DTCOPBuf[DTC_OPTIMIZE_BUFFER_SIZE], 32);
...
static int _oasis_lite_AddFace(uint16_t *faceId, void *faceData, SnapshotItem_t *snapshot, int num, void *userData)
{
...
static int _oasis_lite_UpdateFace(
    uint16_t faceId, void *faceData, SnapshotItem_t *snapshotData, int num, void *userData)
{
...
static void _oasis_start_recognition(oasis_lite_param_t *pParam)
{
    ...
    //Even no LPM support, we should enable recognition/face detection timer for IR/White LED adjustment purpose
