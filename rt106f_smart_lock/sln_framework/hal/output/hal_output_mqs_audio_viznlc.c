...
/* DMA */
#define MQS_DMA            DMA0
#define MQS_DMAMUX         DMAMUX
#define MQS_SAI_EDMA_TX_CH 0
#define MQS_SAI_TX_SOURCE  kDmaRequestMuxSai3Tx
#define MQS_SAI_SAMPLE_RATE kSAI_SampleRate48KHz
#define MQS_SAI_WORD_WIDTH kSAI_WordWidth16bits
#define MQS_SAI_CHANNEL_MASK kSAI_Channel0Mask
#define MQS_SAI_STEREO kSAI_Stereo
#define MQS_PWM_OVERSAMPLE_RATE kIOMUXC_MqsPwmOverSampleRate32
...
#if MQS_SAI_STEREO==kSAI_Stereo
#define MQS_AUDIO_CHUNK_SIZE ((MQS_SAI_SAMPLE_RATE) * (MQS_SAI_WORD_WIDTH/8)  * 2 / 10 )  //100ms
#else
#define MQS_AUDIO_CHUNK_SIZE ((MQS_SAI_SAMPLE_RATE) * (MQS_SAI_WORD_WIDTH/8) / 10 )  //100ms
#endif
...
#define PROMPTS_AUDIO_CHUNK_SIZE_1MS ((MQS_SAI_SAMPLE_RATE) * (MQS_SAI_WORD_WIDTH/8) / 1000)
...
extern "C" {
#endif
static void BOARD_InitMqsResource(void);
...
__attribute__((weak)) int APP_OutputDev_MqsAudio_InputNotifyDecode(
    const output_dev_t *dev, void *inputData, void **audio, uint32_t *len, uint8_t language)
{
    return 0;
}
...
static void _ConfigMqs(void)
{
    ...
    IOMUXC_MQSConfig(IOMUXC_GPR, MQS_PWM_OVERSAMPLE_RATE,
...
static hal_output_status_t HAL_OutputDev_MqsAudio_InputNotify(const output_dev_t *dev, void *data)
{
    ...
    {
        APP_OutputDev_MqsAudio_InputNotifyDecode(dev, data, (void *)&audioBuffer, &audioLen, 1);
        if (audioBuffer != NULL && audioLen != 0)
        {
...
static void BOARD_InitMqsResource(void)
{
    ...
    CLOCK_SetMux(kCLOCK_Sai3Mux, MQS_SAI3_CLOCK_SOURCE_SELECT);
    CLOCK_SetDiv(kCLOCK_Sai3PreDiv, MQS_SAI3_CLOCK_SOURCE_PRE_DIVIDER);
    CLOCK_SetDiv(kCLOCK_Sai3Div, MQS_SAI3_CLOCK_SOURCE_DIVIDER);
}
...
static hal_output_status_t HAL_OutputDev_MqsAudio_Init(output_dev_t *dev, output_dev_callback_t callback)
{
    ...
    SAI_GetLeftJustifiedConfig(&edmaTxConfig, MQS_SAI_WORD_WIDTH, MQS_SAI_STEREO, MQS_SAI_CHANNEL_MASK);
    ...
    SAI_TxSetBitClockRate(MQS_SAI,
    		MQS_SAI_CLK_FREQ,
			MQS_SAI_SAMPLE_RATE,
    		MQS_SAI_WORD_WIDTH,
			MQS_SAI_STEREO==kSAI_Stereo?2U:1U);
...
#endif /* ENABLE_OUTPUT_DEV_MqsAudio_VIZNLC */
