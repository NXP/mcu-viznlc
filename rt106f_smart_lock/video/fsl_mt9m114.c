...
status_t MT9M114_Init(camera_device_handle_t *handle, const camera_config_t *config)
{
    ...
    {
        status = MT9M114_MultiWrite(handle, mt9m114InitConfig_15fps, ARRAY_SIZE(mt9m114InitConfig_15fps));
    }
    if (kStatus_Success != status)
    {
        return status;
    ...
    else if (config->resolution == kVIDEO_ResolutionQVGA)
    {
        status = MT9M114_MultiWrite(handle, mt9m114_qvga, ARRAY_SIZE(mt9m114_qvga));
    }else if (config->resolution == FSL_VIDEO_RESOLUTION(480, 272))
	{
