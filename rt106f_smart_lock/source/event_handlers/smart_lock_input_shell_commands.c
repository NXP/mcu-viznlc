...
static shell_status_t _GetManagerCommand(shell_handle_t shellContextHandle, int32_t argc, char **argv)
{
    ...
    if (argc == 1)
    {
        if (s_InputCallback != NULL)
        {
            ...
            s_InputEvent.u.frameworkRequest       = &s_FrameworkRequest;
            s_InputEvent.size = sizeof(s_FrameworkRequest);
            s_InputEvent.eventId = kInputEventID_FrameworkRecv;
...
static shell_status_t _RenameCommand(shell_handle_t shellContextHandle, int32_t argc, char **argv)
{
    ...
    receiverList                     = (1 << kFWKTaskID_VisionAlgo) | (1 << kFWKTaskID_Output);
...
static shell_status_t _VersionCommand(shell_handle_t shellContextHandle, int32_t argc, char **argv)
{
    ...
    {
    	char verBuf[64];
    	OASISLT_getVersion(verBuf,sizeof(verBuf));
        SHELL_Printf(shellContextHandle, "Oasis version %s\r\n", verBuf);
    }
    else
    {
