...
void BOARD_CSICameraI2CInit(void)
{
    ...
    BOARD_LPI2C_Init(BOARD_CSI_RGB_CAMERA_I2C_BASE, BOARD_CSI_CAMERA_I2C_CLOCK_FREQ);
#if CAMERA_DIFF_I2C_BUS
    BOARD_LPI2C_Init(BOARD_CSI_IR_CAMERA_I2C_BASE, BOARD_CSI_CAMERA_I2C_CLOCK_FREQ);
#endif
}

status_t BOARD_CSICameraRGBI2CSend(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, const uint8_t *txBuff, uint8_t txBuffSize)
{
    return BOARD_LPI2C_Send(BOARD_CSI_RGB_CAMERA_I2C_BASE, deviceAddress, subAddress, subAddressSize, (uint8_t *)txBuff,
                            txBuffSize);
}

status_t BOARD_CSICameraRGBI2CReceive(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, uint8_t *rxBuff, uint8_t rxBuffSize)
{
    return BOARD_LPI2C_Receive(BOARD_CSI_RGB_CAMERA_I2C_BASE, deviceAddress, subAddress, subAddressSize, rxBuff,
                               rxBuffSize);
}

status_t BOARD_CSICameraIRI2CSend(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, const uint8_t *txBuff, uint8_t txBuffSize)
{
    return BOARD_LPI2C_Send(BOARD_CSI_IR_CAMERA_I2C_BASE, deviceAddress, subAddress, subAddressSize, (uint8_t *)txBuff,
                            txBuffSize);
}

status_t BOARD_CSICameraIRI2CReceive(
    uint8_t deviceAddress, uint32_t subAddress, uint8_t subAddressSize, uint8_t *rxBuff, uint8_t rxBuffSize)
{
    return BOARD_LPI2C_Receive(BOARD_CSI_IR_CAMERA_I2C_BASE, deviceAddress, subAddress, subAddressSize, rxBuff,
                               rxBuffSize);
}
...
void board_elcdif_rk024hh298_backlight_control(bool pullUp)
{
    if (pullUp)
    {
...
void BOARD_InitBleQn9090Resource(uint32_t *uartRootClk)
{
    BOARD_InitLPUART5Pins(); //for BLE communication
    BOARD_InitBLEResetPin(); //for BLE reset
    *uartRootClk = CLOCK_GetClockRootFreq(kCLOCK_UartClkRoot);
    //start BLE
    GPIO_PinWrite(BOARD_BLE_QN9090_RESET_GPIO,BOARD_BLE_QN9090_RESET_GPIO_PIN,1);
...
#define BUTTON_PIN  (1 << BOARD_BUTTON_SW1_PIN) | (1 << BOARD_BUTTON_SW2_PIN);
void GPIO3_Combined_0_15_IRQHandler(void)
{
    ...
    intPin = GPIO_PortGetInterruptFlags(BUTTON_GPIO);
    pushButtonInt = BUTTON_PIN;
    ...
    if (xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED)
    {
        ...
        {
            INPUT_DEV_PUSH_BUTTONS_IRQHandler(BUTTON_GPIO, intPin);
            /* clear the interrupt status */
            GPIO_PortClearInterruptFlags(BUTTON_GPIO, pushButtonInt);
