...
//Each bit means a voting for power off
enum{
	AT_COMMAND_FACEREG_IN_PROGRESS,
	AT_COMMAND_FACEDEREG_IN_PROGRESS,
	AT_COMMAND_PARA_UPDATE_IN_PROGRESS
};

static uint32_t s_ATCommandPowerOffStatus = 0;

static uint32_t s_eventIDList4PowerOff[] =
{
...
static TimerHandle_t s_powerOffBlockingTimer = NULL;
...
static sln_at_command_t Parse_ATCommand(uint8_t *rcv_buff, uint32_t buff_len)
{
    ...
    if ((result_str = (uint8_t *)strstr((const char *)rcv_buff, (const char *)"AT+")) != NULL)
    {
        ...
        command_str = (uint8_t *)strstr((const char *)result_str, (const char *)"PWOFFREQ=");
        if (command_str != NULL)
        {
            return PWOFFREQ;
        }
...
static int Send_ATCommnd_Response(sln_at_command_t command, char *value)
{
    ...
    switch (command)
    {
        ...
        case PWOFFRSP:
            size = snprintf(s_response_buff, AT_COMMANDS_RSP_BUFF_SIZE, "AT+PWOFFRSP=%s\r\n", value);
            break;
        default:
            size = -1;
            break;
    ...
    switch (eventId) {
        ...
        case kEventFaceRecID_AddUserRemote:
            if (status == kEventStatus_Ok)
            {
                ...
                {
                    Send_ATCommnd_Response(FACERREG_RSP, res->name);
                }
        ...
        switch (command)
        {
            case FACEREG:
                ...
                if (s_InputDev_ATCommands->cap.callback != NULL)
                {
                    ...
                    s_InputEvent.u.inputData.data           = &s_ATCommandsEvent;
                    s_InputEvent.u.inputData.receiverList = receiverList;
					s_InputEvent.size                     = sizeof(s_ATCommandsEvent);
					s_InputEvent.eventId = kInputEventID_Recv;
                    s_InputDev_ATCommands->cap.callback(s_InputDev_ATCommands, &s_InputEvent, fromISR);
            ...
            case FACEDEL:
                ...
                break;
            case PWOFFREQ:
            	Send_ATCommnd_Response(PWOFFRSP, s_ATCommandPowerOffStatus == 0?"ACK":"NACK");
...
void APP_OutputDev_ATCommands_InferCompleteDecode(output_algo_source_t source, oasis_lite_result_t inferResult)
{
    ...
    switch (inferResult.state)
    {
        ...
        case kOASISLiteState_Registration:
        	if (result == kOASISLiteRegistrationResult_Invalid)
        	{
            	//it means a start of registration
            	s_ATCommandPowerOffStatus |= 1UL << AT_COMMAND_FACEREG_IN_PROGRESS;
        	}else
        	{
            	s_ATCommandPowerOffStatus &= ~(1UL << AT_COMMAND_FACEREG_IN_PROGRESS);
        	}
        ...
        case kOASISLiteState_DeRegistration:
        	if (result == kOASISLiteDeregistrationResult_Invalid)
        	{
            	//it means a start of registration
            	s_ATCommandPowerOffStatus |= 1UL << AT_COMMAND_FACEDEREG_IN_PROGRESS;
        	}else
        	{
            	s_ATCommandPowerOffStatus &= ~(1UL << AT_COMMAND_FACEDEREG_IN_PROGRESS);
        	}
...
{
	s_ATCommandPowerOffStatus &= ~(1UL << AT_COMMAND_PARA_UPDATE_IN_PROGRESS);
}

void APP_OutputDev_ATCommands_InputNotify(uint32_t eventID)
{
    if (s_powerOffBlockingTimer == NULL)
    {
        s_powerOffBlockingTimer = xTimerCreate(NULL, pdMS_TO_TICKS(BLOCKING_TIME_MS), pdFALSE,
                                    (void *)0, _BlockingTimerCallback);
    }

    for(uint32_t i = 0;i<sizeof(s_eventIDList4PowerOff);i++)
    {
    	if (eventID == s_eventIDList4PowerOff[i])
    	{
    		//delay 2 seconds for power on request
    		s_ATCommandPowerOffStatus |= 1UL << AT_COMMAND_PARA_UPDATE_IN_PROGRESS;
    		xTimerStart(s_powerOffBlockingTimer, 0);
    		break;
    	}
    }
}
